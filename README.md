...
Этот мануал описывает шаги для установки и настройки скрипта **Mihomo Auto VLESS Reality Updater**, который автоматически обновляет конфигурацию прокси-серверов для Mihomo и генерирует файл с серверами VLESS.
...
4. 🔧 [**Ручная установка**](#4-ручная-установка)
   ...
   * 4.1 [Установка зависимостей](#41-установка-зависимостей)
   * 4.2 [Создание и настройка скрипта](#42-создание-и-настройка-скрипта)
   * 4.3 [Настройка прав доступа](#43-настройка-прав-доступа)
   * 4.4 [Ручной запуск скрипта](#44-ручной-запуск-скрипта)
5. 🗣️ [**Обновление через API**](#5-обновление-через-api)

---

## 1. 📝 Введение

**Mihomo Auto VLESS Reality Updater** — это скрипт для автоматической загрузки, проверки и обновления конфигурации прокси-серверов в формате YAML для Mihomo. Скрипт скачивает актуальные данные о прокси-серверах, проверяет их на задержку, страну и обновляет конфигурационный файл. Обновление происходит каждые 30 минут, и файл yaml перезаписывается только в случае изменения набора ключей.

⚠️ **Важно**: перед установкой убедитесь, что на вашем устройстве уже установлены **Entware** и **Mihomo**. Скрипт работает только при наличии этих программ на устройстве.
...
* ⚙️ **Устанавливает все зависимости**, включая `curl`, `openssl`, `gawk`, `vim` и другие утилиты.
...
После выполнения этой команды все компоненты будут установлены, а ваш скрипт настроен и готов к использованию. Создан бекап вашего config.yaml, в той же папке.
...
### 4.4 Ручной запуск скрипта

Теперь вы можете вручную запустить скрипт, чтобы проверить его работу:
...
## 5. 🗣️ Обновление через API

В случае, если вы хотите обновить конфигурацию прокси-серверов через API, используйте команду:

```sh
curl -s -X POST http://127.0.0.1:9090/proxies
```

После этого Mihomo автоматически загрузит новую конфигурацию.
